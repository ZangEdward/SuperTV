...
- 📱 **投屏交互大升级**：
    - **双端同步**：手机端进度条与按键现可实时控制电视/投屏设备的播放进度及选集切换。
    - **自动连播**：投屏状态下支持自动换集，当电视端播放接近结束时，手机端将自动下发下一集投屏指令。
    - **完全控制**：支持通过手机端控制投屏设备的暂停、播放、进度跳转以及优雅退出。
    - **状态同步**：优化了投屏状态下的心跳同步逻辑，确保手机 UI 实时反映远程设备的播放状态。
- 📺 **TV 交互优化**：在首页浏览海报时，按遥控器“返回”键现可直接回到顶部并聚焦当前子分类，极大提升选片效率。
- 📺 **焦点逻辑修复**：修复了 TV 端顶部“视频/直播”切换按钮在向右导航时焦点短暂丢失的问题。通过显式建立焦点链接并优化容器点击穿透逻辑，确保了导航的连贯性。
- 📺 **播放器视觉优化**：播放器在进入全屏播放模式后，背景色自动切换为全黑色（#000），消除视频两侧可能出现的灰边，提供更沉浸的观影体验。
- 🚀 **线路一键优化升级**：重构了播放源优化逻辑。现在点击“一键优化”后，系统将并发对所有播放源进行深度测速（下载第一集首个 TS 片段前 2MB 数据），精确测量延迟与下载速度，并自动切换至最快线路。
