...
- 🗑️ **移除冗余功能**：正式移除已废弃的「播放源管理」功能，简化设置项，全面转向由「一键优化」驱动的自动化线路管理。
- 🚀 **新增「盘搜」功能**：集成全网网盘资源搜索，支持**夸克网盘**、**磁力链接**、**百度网盘**分类，后续加入其他网盘展示。
- 📺 **TV 交互大优化**：
    - 首页选中海报时按“返回”键可快速回到顶部并聚焦当前分类。
    - 修复了顶部导航在向右滑动时焦点丢失的问题。
- 🛠️ **投屏控制增强**：支持通过手机端实时控制电视播放进度、换集、暂停/播放及优雅断开。
- ⚡ **线路优化升级**：重构测速逻辑，支持并发深度测速（2秒超时），自动优选最快线路。
