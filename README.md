...
📱 v5.5.22.523 (Latest)
- 🔄 **移动端旋转优化**：移动端现在支持根据手机方向自动旋转，不再强制锁定竖屏。
- 🎬 **最近播放直达**：点击最近播放记录直接进入播放页面，无需经过详情页，提升效率。
- 🔍 **搜索性能优化**：搜索结果实时动态加载，并按标题全局去重，大幅提升搜索响应速度和结果整洁度。
- 📁 **缓存修复尝试**：针对部分设备下载失败问题，优化了文件系统路径处理逻辑。
- 🏗️ **版本号升级**：全线同步至 `v5.5.22.523`。
