...
- [x] **导航交互优化**：
    ...
    - **登录访问体系**：实现 Selene 风格登录拦截流，强制未登录用户完成验证以保护服务器资源。
    - **乱码彻底修复**：在 Gradle 编译阶段和 App 运行时均强制使用 **UTF-8** 编码，彻底解决 API 节点中文乱码问题。
    - **自动 401 拦截**：集成 API 监听，当后端提示登录失效（401）时，App 将自动弹出登录窗提示重新登录。
    - **登录 UI 优化**：登录界面支持从预设节点菜单中快速选择，仅显示节点名称，隐藏复杂网址。支持返回跳过进入首页。
    - **深色/浅色模式适配**：重构了全站主题系统，所有字体和背景颜色现在都能完美匹配系统的深色或浅色模式。
    - **自适应布局 (Tablet/Mobile)**：
        - **平板模式**：导航栏自动移动至屏幕左侧（Sidebar），优化大屏下的内容展示。
        - **移动端**：完美适配横竖屏切换，加入 Edge-to-Edge 全屏显示与安全区避让，彻底解决 UI 遮挡问题。
    - **剧集卡片升级**：全面对齐 Selene 风格，加入豆瓣评分显示、来源标签、年份标识，视觉更加精致。
    - **混合认证支持**：修复了 401 登录失败问题。
    - **乱码彻底修复**：解决 `api_nodes.json` 中文显示异常。
