...
📱 v5.5.25.523 (Latest)
- ⚙️ **设置页面优化**：将缓存管理移至设置项顶部，提升操作便捷性。
- 🔍 **搜索体验提升**：优化搜索调度算法，支持结果异步流式刷新，搜索速度显著加快；增加全局去重机制，同剧集不再显示重复结果。
- ⬇️ **下载任务批量管理**：缓存管理新增“全部暂停”与“全部启动”功能，方便批量控制下载任务。
- ⚡ **性能优化**：取消启动时的自动节点测速，改用智能首选机制，大幅缩短应用冷启动时间。
- 📂 **断点续传与持久化**：完善下载队列持久化，支持退出 APP 后自动续传。
