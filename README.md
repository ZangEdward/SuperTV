...
- **Bangumi 增强**:
  - **服务器代理**: 针对国内网络环境，引入服务器节点代理模式，支持对 Bangumi 放送日历、番剧详情及封面图片的转发请求，确保数据稳定获取。
  - **按需转发**: 代理模式仅作用于 Bangumi 相关接口，豆瓣（Douban）及其他视频源仍保持原始直连或 CDN 模式，兼顾效率与兼容性。
- **登录优化**: 
  - **智能节点**: 登录界面集成服务器节点下拉选择框，支持通过环境变量 `API_NODES_JSON` 动态注入多个后端入口。
  - **全主题支持**: 登录界面已适配深浅模式，配色方案与全局主题保持高度一致，提供连贯的视觉体验。
