...
编译好的版本无法使用，为储存源代码用，有兴趣可以安装后自己修改。
## ✨ 功能特性
...
- 🚀 **搜索体验升级（借鉴 Selene-Source）**：
    - **增量式异步加载**：搜索结果不再需要等待所有源完成，采用“流式”反馈，搜到一个出一个，极速响应。
    - **动态搜索进度**：新增可视化搜索进度条，实时显示当前检索源及完成比例。
- ⚡ **测速算法重构（高性能模式）**：
    - **小样本并发 RTT**：引入 Selene-Source 的高性能测速模型，通过 1KB Range 请求极速探测延迟，测速效率提升 5 倍。
    - **加权评分排序**：综合考虑下载速度（40%）、网络延迟（30%）、分辨率（20%）及稳定性（10%），为用户智能优选最佳线路。
    - **智能分批调度**：每 3 个源为一组进行滑动窗口并发测速，避免带宽竞争，确保结果真实准确。
- 🔍 **选集名称深度优化**：
    - **真实标题适配**：优先展示源接口返回的剧集名称（如：第20240520期），仅在无数据时回退至“第X集”。
    - **智能 UI 布局**：详情页选集区域支持长标题自动变宽，完美适配综艺及纪录片长标题展示。
