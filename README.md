...
📱 v5.5.23.523 (Latest)
- 🐛 **修复最近播放直达崩溃**：针对 direct-play 逻辑增加了多层防御性检查，防止在搜索未命中或延迟加载时触发 Native 崩溃（图标闪现问题）。
- ⬇️ **缓存下载稳定性增强**：重写了高性能 Base64 编码算法，规避了大文件下载时的内存栈溢出问题，提升了 M3U8 合并的成功率。
- 🏗️ **版本号升级**：全线同步至 `v5.5.23.523`。
