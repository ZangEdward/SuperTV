...
- 🚀 **播放器手势大升级**：
    - **滑动控进度**：在播放界面左右滑动可快速调节播放进度（1px = 200ms）。
    - **双击启停**：双击播放区域实现暂停/播放切换。
    - **全黑模式**：TV 端及全屏模式下背景自动切换为全黑，消除视觉干扰。
- 🚀 **「盘搜」深度集成**：
    - **多网盘支持**：集成夸克、磁力、百度、阿里、迅雷等网盘搜索。
    - **智能分类**：结果按网盘类型自动分类展示，且**仅显示有结果的类别**，空结果分类将自动隐藏。
    - **移动端独享**：该功能仅在移动端及平板端可见，优化了资源发现路径。
    - **便捷操作**：支持一键复制链接及直接唤起相关 App。
- 📺 **TV 交互微调**：
    - **返回键直达**：海报列表按返回键直接回到顶部聚焦分类。
    - **焦点路径修复**：解决顶部导航在特定方向滑动时焦点丢失的问题。
- 🛠️ **稳定性增强**：
    - **系统更新修复**：修复了下载完安装包后无法自动唤起系统安装程序的问题，优化了文件权限共享逻辑。
    - **进度条拖拽**：针对平板端优化了进度条触摸响应，支持精准拖拽。
    - **性能优化**：并发深度测速逻辑升级，2秒超时自动切换最快线路。
    - **UI 精简**：移除搜索、收藏、设置页面的多余返回按钮。
    - **冗余清理**：正式删除已废弃的播放源管理页面。
