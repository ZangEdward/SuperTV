...
class ServerNode {
  ...
  ServerNode({required this.key, required this.label, required this.url});
    ...
    if (nodesJson.isNotEmpty) {
      try {
        ...
        setState(() {
          ...
          // 如果有节点，尝试根据当前 url 选中
          if (_serverNodes.isNotEmpty) {
            final currentUrl = _urlController.text;
            _selectedNode = _serverNodes.firstWhere(
              (node) => node.url == currentUrl,
              orElse: () => _serverNodes.first,
            );
            if (_selectedNode != null && _urlController.text.isEmpty) {
              _urlController.text = _selectedNode!.url;
            }
          }
        });
      } catch (e) {
        debugPrint('Error parsing API_NODES_JSON: $e');
      }
    }
  }
  ...
  void _loadSavedUserData() async {
    ...
    if (userData['serverUrl'] != null) {
      ...
      if (_serverNodes.isNotEmpty) {
        setState(() {
          _selectedNode = _serverNodes.firstWhere(
            (node) => node.url == userData['serverUrl'],
            orElse: () => _serverNodes.first,
          );
        });
      }
  ...
  void _handleLogoTap() {
    ...
    if (_logoTapCount >= 10) {
      ...
      _showToast(
        ...
        _isLocalMode ? const Color(0xFF27ae60) : const Color(0xFF3498db),
      );
    } else {
      // 设置新的计时器，2秒后重置计数
